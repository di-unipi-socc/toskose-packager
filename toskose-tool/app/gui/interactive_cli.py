...
class InteractiveCLI():  
    ...
    def __init__(self):
        ...
        self._paths = InteractiveCLI.SessionPaths()
        self._flags = InteractiveCLI.SessionFlags()
    ...
    def __action_handler(self):
        ...
        try:
            ...
            self.notify = ('An fatal error is occurred', Alerts.Failure)
    ...
    def tosca_load_from(self, source='File'):
        ...
        while True:
            ...
            if (local_notify is not None):
                InteractiveCLI.print_alert(*local_notify)
                local_notify = None
            ...
            try:
                ...
                manifest_path = (os.path.join(tl.csar_dir_path, tl.manifest_file))
                
                # Validation
                ToscaValidator(manifest_path).validate()
                self._flags.validated = True
                self._paths = InteractiveCLI.SessionPaths(**tl.get_paths())
                self._csar_metadata = tl.csar_metadata

                # Build Model
                self._tosca_model = ToscaModel(manifest_path)
                self._flags.topology = True

                break
            
            except (ToscaFileNotFoundError, ToscaParsingError, ToscaValidationError) as err:
                local_notify = (err, Alerts.Failure)
            except Exception as err:
                logger.exception(err)
                self.notify = ('A fatal error is occurred', Alerts.Failure)
                break
             
        self.go_main_menu() 

    def tosca_metadata(self):
        self.notify = ('Not implemented yet', Alerts.Warning)
    ...
    def print_notification(self):
        if self.notify is not None:
            InteractiveCLI.print_alert(self.notify[0], self.notify[1])
            self.notify = None 
