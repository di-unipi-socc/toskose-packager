...
def separate_full_image_name(full_name):
    ...
    if len(splitted) == 3:
        ...
        raise FatalError('Cannot separate {} Docker image full name'.format(
            full_name))
...
class DockerManager():
        ...
        for line in result:
            ...
            if 'error' in line:
                ...
                if 'denied' or 'unauthorized' in line['error']:
                    logger.info(
                        'Access to the repository denied. \
    ...
    def _remove_previous_toskosed(self, image, tag=None):
        ...
        try:
            ...
            logger.debug('Image [{0}] found. It\'s referenced by the following \
                tags:\n\n{1}'.format(
                image,
    ...
    def toskose_image(self, src_image, src_tag, dst_image, dst_tag, context,
                      process_type, app_name, toskose_dockerfile=None,
                      toskose_image=None, toskose_tag=None, enable_push=True):
        ...
        try:
            ...
            build_args = {
                'TOSCA_SRC_IMAGE': '{0}:{1}'.format(
                    src_image,
                    src_tag),
                'TOSKOSE_BASE_IMG': '{0}:{1}'.format(
                    toskose_image,
                    toskose_tag)
            }
